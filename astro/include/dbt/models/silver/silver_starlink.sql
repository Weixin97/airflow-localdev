{{ config(
    materialized='view',
    description='Cleaned Starlink satellite data with status determination and orbital parameters'
) }}
...
{{ config(
    post_hook="COMMENT ON VIEW {{ this }} IS 'Cleaned Starlink satellite data with active status determination and orbital parameters'"
) }}