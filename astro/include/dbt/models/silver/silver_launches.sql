{{ config(
    materialized='view',
    description='Cleaned and parsed launch data with business logic applied'
) }}
...
{{ config(
    post_hook="COMMENT ON VIEW {{ this }} IS 'Cleaned launch data with Starlink mission identification and satellite count estimation'"
) }}